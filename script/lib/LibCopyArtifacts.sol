...
library LibCopyArtifacts {
    ...
    function contracts() internal pure returns (string[] memory) {
        ...
        names[0] = "DecimalFloat";
        names[1] = "TestDecimalFloat";
        return names;
    }
    ...
    function committedPath(string memory contractName) internal pure returns (string memory) {
        return string.concat("crates/float/abi/", contractName, ".json");
    }
    ...
    /// generation and the bytecode fields are consumed by the rust EVM
    /// setup in `crates/float/src/evm.rs`.
